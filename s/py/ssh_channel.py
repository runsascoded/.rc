...
def get_socket_for_host(host):
    sockets = [control_masters_dir +
               socket for socket in os.listdir(control_masters_dir) if host in socket]
    if len(sockets) > 1:
        raise Exception(
            'Found multiple sockets for host %s:\n%s' % (host, sockets.join('\n')))
    if len(sockets):
        return sockets[0]
    return None
...
class SshChannel:
    ...
    def __init__(self, host, user=None):
        ...
        if Popen(['ssh', '-O', 'check', host]).wait() == 0:
            ...
            if not self.socket:
                raise Exception(
                    'Expected to find ControlMaster for host %s, but didn\'t find one at %s' % (
                        host, control_masters_dir)
                )
            return
        ...
        self.channel = Popen(
            ['ssh', '-M', '-S', self.socket, host_str], stdin=PIPE, stdout=PIPE, stderr=PIPE)
        first_sleep = True
        sleep_time = .1
