...
# Remove local git module from path to avoid import conflict
sys.path = [p for p in sys.path if not p.endswith('/.rc')]
...
def generate_submodule_commit_message(repo, changed_submodules):
    """Generate commit message for submodule changes (like g saca does)."""
    messages = []

    for submodule in sorted(changed_submodules):
        submodule_path = Path(repo.working_dir) / submodule
        if not submodule_path.exists():
            continue

        # Get the submodule repo
        try:
            sub_repo = Repo(submodule_path)
            # Count commits since last update
            # This is a simplified version - you might want to match your exact g saca logic
            log = sub_repo.git.log('--oneline', '-n', '20')
            commit_count = len(log.split('\n')) if log else 0
            messages.append(f"{submodule}+{commit_count}")
        except Exception as e:
            err(f"Error processing submodule {submodule}: {e}")
            messages.append(f"{submodule}+?")

    return ', '.join(messages) if messages else "Update submodules"
        ...
        if repo.index.diff('HEAD'):
            ...
            with env(
                ...
                repo.index.commit(
                    msg,
                    author=author,
                    author_date=author_date.strftime('%Y-%m-%d %H:%M:%S %z')
                )
